...
class TestGenConfig:
    ...
    max_revisits: int = 2
    max_paths: int = 100
    framework: str = "junit"  # "junit" or "testng"

    def __post_init__(self) -> None:
        if self.framework not in ("junit", "testng"):
            raise ValueError(f"Unknown test framework: {self.framework!r} (expected 'junit' or 'testng')")
...
def generate_test_source(
    ...
    """Generate test class source from a state space.

    Supports JUnit 5 (default) and TestNG frameworks via config.framework.
    """
    result = enumerate(ss, config)
    programs, cp_truncated = enumerate_client_programs(
        ss, config.max_revisits, config.max_paths)
    testng = config.framework == "testng"
    lines: list[str] = []
    ...
    for p in result.incomplete_prefixes:
        ...
        if testng:
            lines.append(
                f'    @Test(enabled = false, description = '
                f'"Incomplete: object not in terminal state after {labels_str}")'
            )
        else:
            lines.append(
